...
def bitrix24_handler(request):
    ...
    event = data.get('event', 'ONCRMLEADADD')
    ...
    if event in ["ONCRMLEADADD", "ONCRMLEADUPDATE"]:
        ...
        if lead_id and lead_name:
            send_to_google_ads(lead_id, lead_name, lead_status, gclid=None)
        else:
            print("Incomplete lead data received")

    elif event == "ONCRMLEADDELETE":
        lead_id = details.get('FIELDS', {}).get('ID')
        print(f"Lead deleted: ID={lead_id}")

    return JsonResponse({"status": "success"})

def send_to_google_ads(lead_id, lead_name, lead_status=None, gclid=None):
    ...
    try:
        ...
        # Uncomment and set the conversion time and value if needed
        ...
        if response.partial_failure_error:
            ...
            print("Conversion successfully uploaded.")
